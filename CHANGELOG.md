...
- Type generation scripts (including `polkadot-types-from-{chain,defs}`) has been moved to the `@chainx-v2/typegen` package
...
- **Breaking change** The metadata classes have been moved from `@polkadot/types` into `@chainx-v2/metadata`. If you explicitly create `Metadata` classes, you now would need to do so with the `import Metadata from '@chainx-v2/metadata';` This is a decorated class, where the previous metadata is now available on `Metadata.metadata`, alongside decorated `consts.*`', `tx.*` and `query.*`.
...
- The `ContractsAbi` type has been moved from `@polkadot/types` to `import { Abi } from '@chainx-v2/api-contract`. This paves the way for an enhanced contracts interface, instead of dealing with low-level API calls.
