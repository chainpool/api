...
function fromSeed(type, seed) {
  return {
    ecdsa: () => (0, _crypto.secp256k1KeypairFromSeed)(seed),
    ed25519: () => (0, _crypto.naclKeypairFromSeed)(seed),
    sr25519: () => (0, _crypto.schnorrkelKeypairFromSeed)(seed)
  }[type]();
}
...
function sign(type, message, pair, {
  ...
  const signature = {
    ecdsa: () => (0, _crypto.secp256k1Sign)(message, pair),
    ed25519: () => (0, _crypto.naclSign)(message, pair),
    sr25519: () => (0, _crypto.schnorrkelSign)(message, pair)
  }[type]();
...
function verify(type, message, signature, publicKey) {
  return {
    ecdsa: () => (0, _crypto.secp256k1Verify)(message, signature, (0, _crypto.blake2AsU8a)(publicKey, 256)),
    ed25519: () => (0, _crypto.naclVerify)(message, signature, publicKey),
    sr25519: () => (0, _crypto.schnorrkelVerify)(message, signature, publicKey)
  }[type]();
}

function getAddress(type, publicKey) {
  if (type === 'ecdsa' && publicKey.length > 32) {
    return (0, _crypto.blake2AsU8a)(publicKey, 256);
  } else {
    return publicKey;
  }
...
function createPair({
  ...
  return {
    ...
    derive: (suri, meta) => {
      ...
      const {
        path
      } = (0, _crypto.keyExtractPath)(suri);
      const derived = (0, _crypto.keyFromPath)({
        publicKey,
        secretKey
      }, path, type);
      return createPair({
