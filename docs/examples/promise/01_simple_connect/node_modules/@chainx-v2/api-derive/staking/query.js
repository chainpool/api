...
function parseSessionIds(stashId, queuedKeys, nextKeys) {
...
function parseController(stashId, queuedKeys, [controllerIdOpt, nominatorsOpt, rewardDestination, validatorPrefs, nextKeys, exposure], stakingLedgerOpt) {
  const controllerId = controllerIdOpt.unwrapOr(null);
  const nominators = nominatorsOpt.unwrapOr(null);
  return controllerId && stakingLedgerOpt.isSome ? _objectSpread({
    accountId: stashId,
    controllerId,
    exposure,
...
function retrieveCurr(api, stashIds, activeEra) {
  ...
  return (0, _rxjs.combineLatest)([api.query.staking.bonded.multi(stashIds), api.query.staking.nominators.multi(stashIds), api.query.staking.payee.multi(stashIds), api.query.staking.validators.multi(stashIds), ((_api$consts$session = api.consts.session) === null || _api$consts$session === void 0 ? void 0 : _api$consts$session.dedupKeyPrefix) ? api.query.session.nextKeys.multi(stashIds.map(stashId => [api.consts.session.dedupKeyPrefix, stashId])) : api.query.session.nextKeys.multi(stashIds), api.query.staking.erasStakers.multi(stashIds.map(stashId => [activeEra, stashId]))]).pipe((0, _operators.map)(([controllerIdOpt, nominatorsOpt, rewardDestination, validatorPrefs, nextKeys, exposure]) => controllerIdOpt.map((controllerIdOpt, index) => [controllerIdOpt, nominatorsOpt[index], rewardDestination[index], validatorPrefs[index], nextKeys[index], exposure[index]])));
}
...
function query(api) {
  return (0, _util2.memo)(accountId => api.derive.staking.queryMulti([accountId]).pipe((0, _operators.map)(([first]) => first)));
}

function queryMulti(api) {
  return (0, _util2.memo)(accountIds => accountIds.length ? (0, _rxjs.combineLatest)([api.query.session.queuedKeys(), api.derive.session.indexes()]).pipe((0, _operators.switchMap)(([queuedKeys, {
    activeEra
  }]) => {
    const stashIds = accountIds.map(accountId => api.registry.createType('AccountId', accountId));
    return ((0, _util.isFunction)(api.query.staking.erasStakers) ? retrieveCurr(api, stashIds, activeEra) : (0, _rxjs.combineLatest)(stashIds.map(stashId => retrievePrev(api, stashId)))).pipe((0, _operators.switchMap)(results => retrieveControllers(api, results.map(([optController]) => optController)).pipe((0, _operators.map)(stakingLedgerOpts => stashIds.map((stashId, index) => parseController(stashId, queuedKeys, results[index], stakingLedgerOpts[index]))))));
  })) : (0, _rxjs.of)([]));
}